...
def ParseBugPages(filesq,procId,q):
    ...
    while not filesq.empty():
        ...
        data,stat = Helpers.getBugData(file.replace('.html',''),Helpers.ExtractBugPatches)
        if stat < 0:
...
if __name__ == '__main__':
    ...
    for file in bugfiles: 
        filesq.put(file)
    numproc = 20
    #lnfiles = len(bugfiles)
    #nmp = int(lnfiles/numproc)+1
    ...
    ret = dbh.CleanInsertMany(datarows,keys,table='PATCHES')
